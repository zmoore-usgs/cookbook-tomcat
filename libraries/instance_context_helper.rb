...
class Chef
  class Recipe
    # Normalizes incoming context values into something that downstream functions expect
    class ContextHelper
      # Runs every element in the incoming resources array through the
      # normalize function
      def self.normalize_resources(incoming_resources)
        incoming_resources.map { |r| normalize_resource(r) }
      end

      def self.normalize_environments(incoming_environments)
        incoming_environments.map { |e| normalize_environment(e) }
      end
