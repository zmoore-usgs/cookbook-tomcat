...
if node["wsi_tomcat"]["deploy"]["remove_unlisted_applications"]
  instances.each do |instance, attributes|
    if attributes.has_key?("application")
      node["wsi_tomcat"]["instances"][instance]["service_definitions"].each do |sd, sd_attr|
        ...
        deployed_apps = Array.new
        success = false
        begin
          open("http://127.0.0.1:#{port}/manager/text/list", :http_basic_authentication=>["tomcat-script","#{tomcat_script_pass}"]) do |f|
            # Format coming back
          ...
          if success
            deployed_apps.each do |appname, attr|
              # Don't delete the manager app if it's supposed to exist
              if appname != "manager" || (appname == "manager" && node["wsi_tomcat"]["disable_manager"])
                if ! attributes.application.keys.include? appname
                  wsi_tomcat_instance instance do
